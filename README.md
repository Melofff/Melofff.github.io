...
|화면 구성|시작 화면, 게임 화면, 클리어 화면으로 구성|
|:----:|:----:|
|시작화면|1.시작 버튼, 종료 버튼 존재<br>2.시작 버튼 - 게임 화면으로 이동<br>종료 버튼 - 게임 종료|
|게임 화면|1.현재 체력, 스태미나, 무기를 좌상단에 표시<br>2.중앙에 플레이어 캐릭터 표시<br>3.스테이지는 총 5가지 - 중앙 스테이지 기준 0, 90, 180, 270도 방향마다 각 스테이로 향하는 문 존재<br>4.조작은 WASD - 이동, 마우스 좌클릭 – 공격, 마우스 이동 – 시점 회전<br>5.스페이스바 - 회피, E키 – 물약 사용<br>6.Esc키 – 게임 종료 버튼 표시, 버튼 클릭시 게임 저장 및 종료<br>7.평소엔 마우스를 숨기지만 esc를 누르면 마우스를 표시<br>8.플레이어가 사망하거나 스테이지 보스가 사망하면 중앙 스테이지로 플레이어 이동<br>9.모든 스테이지를 클리어하고 후, 중앙 스테이지의 제단과 상호 작용시 게임 클리어 - 클리어 화면으로 이동|
|클리어|1.“시작 화면으로” 버튼 존재 - 클릭시 시작 화면으로 이동. 보스 클리어 기록 초기화.|
