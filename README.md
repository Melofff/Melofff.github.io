...
|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|flaot|MaxHealth|데미지를 전혀 입지 않은 상태의 체력 값.<br>이 이상 현재 체력은 높아지지 않음||
|flaot|CurrentHealth|적의 공격을 얼마나 견딜 수 있는지 나타내는 수치.<br>0이 되면 사망||
|flaot|MaxStamina|행동을 전혀 하지 않은 상태의 스태미나 값.<br>이 이상 현재 스태미나는 높아지지 않음||
|flaot|CurrentStamina|캐릭터의 행동에 소모되는 값.<br>행동 필요 수치보다 낮을 시 해당 행동 불가.||
|flaot|Speed|캐릭터의 기본 이동 속도.||
|enum|Weapon|현재 사용 중인 무기를 나타내는 수치||
|bool[]|haveWeapon|현재 소유 중인 무기. false인 무기는 무기 변경시 건너 뜀||
...
|속성|영문 명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|bool[ ]|isClear|각 스테이지 클리어 여부를 나타내는 값||
