...
|피해량|Damage|몬스터의 공격이 플레이어 캐릭터에게 접촉했을 시,<br>플레이어 캐릭터의 체력을 감하는 수치||
...
### c. 보스_북 패턴
  - 조우 : 포효 후, 브래스
  - 거리가 돌진 거리 이상이면 이동, 대기 중 무작위 행동
  - 거리가 돌진 거리 미만, 공격 거리 이상이면 대기, 돌진, 이동 중 무작위 행동
  - 거리가 공격 거리 미만이면 할퀴기, 깨물기, 돌진, 대기 중 무작위 행동
  - 체력이 50% 미만이 되면 할퀴기 대신 연속 할퀴기 사용
  - 체력이 50% 미만이 되면 비행 후 맵의 무작위 장소에 착지 이후 위 패턴 반복
  - 체력이 0이 되면 사망.
...
## 10-1. 1주차
### 캐릭터 이동 구현
#### ~~WASD : 카메라 기준 전진 후진, 좌우~~
## 10-2. 2주차
### 캐릭터 행동 구현
#### 1) ~~Space바 : 캐릭터가 바라보는 방향으로 구르기 애니메이션. 스태미나 필요. 회피 애니메이션중엔 피격 당하지 않음~~
#### 2) ~~좌클릭 : 캐릭터의 무기에 따라 공격 애니메이션. 스태미나 필요. 공격 범위에 몬스터가 있으면 무기 데미지 입힘~~
#### 3) ~~E키 : 물약 사용. 물약 애니메이션. 물약 회복량 만큼 일정한 속도로 체력 회복. 최대 체력 이상으로 회복되지 않음~~
#### 4) ~~몬스터 공격과 접촉 시, 피격 애니메이션 재생~~
## 10-3. 3주차
### 보스 ai 구현(시간 관계상 1가지 보스만 구현)
#### 1) 보스 모델 : 북쪽 보스 모델 사용
#### 2) 패턴
  - 조우 : ~~포효 후, 브래스~~
  - ~~거리가 돌진 거리 이상이면 이동, 대기 중 무작위 행동~~
...
#### 2) 게임 화면
  - ~~체력바 : 매 프레임마다 플레이어 최대 체력과 현재 체력의 퍼센트만큼 길이 갱신~~
  - ~~기력바 : 매 프레임마다 플레이어 최대 기력과 현재 기력의 퍼센트만큼 길이 갱신~~
  - ~~물약 아이콘 : 매 프레임마다 물약의 갯수 갱신~~
## 10-5. 5주차
### UI 개발
#### 1) 시작 화면
  - 화면 중앙에 위에서부터 타이틀, 시작 버튼, 종료 버튼
  - 시작 버튼 클릭 시, 게임 화면(중앙 스테이지)으로 화면 전환
  - 종료 버튼 클릭 시, 게임 종료

### 2) 스테이지 환경 조성
  - 스테이지_중앙
  - 천마총 내부와 흡사한 구조
  - 출입구로 나가면 몬스터가 존재하는 스테이지로 이동.
  - 스테이지_북
  - 돔 형태의 맵
  - 사방이 바위로 막혀 있고 천장에 빛나는 바위가 존재.
  - 맵 중앙에 몬스터 배치
